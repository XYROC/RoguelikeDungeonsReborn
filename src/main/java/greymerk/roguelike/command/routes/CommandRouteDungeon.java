...
public class CommandRouteDungeon extends CommandRouteBase {
	...
	public void execute(ICommandContext context, List<String> args) {
		...
		if (settingName != null) {
			...
			try {
				context.sendMessage(
						"Success: \"" + settingName + "\" Dungeon generated at " + dungeon.getPosition().toString(),
						MessageType.SUCCESS);
			} catch (Exception e) {
	...
	public static Coord getLocation(ICommandContext context, List<String> args)
			throws NumberInvalidException, PlayerNotFoundException {
		ArgumentParser ap = new ArgumentParser(args);
